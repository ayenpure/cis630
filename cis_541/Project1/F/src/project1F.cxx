...
Matrix getCameraTransformMatrix(Camera camera) {
	double z_vector[3] = {camera.position[0] - camera.focus[0],
												camera.position[1] - camera.focus[1],
												camera.position[2] - camera.focus[2]};
	normalize_vector(z_vector);
	double x_vector[3],y_vector[3];
	/*memcpy(y_vector, camera.up, 3 * sizeof(double));
	normalize_vector(y_vector);
	cross_product(y_vector, z_vector, x_vector);
	normalize_vector(x_vector);
	if(dot_product(y_vector, z_vector) == (double)0) {
		/*
		 * Case where the up and O-focus are not perpendicular.
		 */
		/*v1 = Up x (O-focus)
		v2 = (O-focus) x v1*/
		cross_product(camera.up, z_vector, x_vector);
		normalize_vector(x_vector);
...
Matrix getViewTransformMatrix(Camera camera) {
	double view_transform[4][4] = {
		cot(camera.angle/2), 0, 0 , 0,
		0, cot(camera.angle/2), 0 , 0,
